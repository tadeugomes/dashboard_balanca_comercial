...
# 2. Função para carregar dados de forma eficiente
ler_dados <- function(nome_arquivo) {
  ...
  tryCatch({
    dados <- arrow::read_parquet(
      caminho_arquivo,
      col_select = everything()  # Seleciona todas as colunas
    )
    
    # Transformações específicas para arquivos NCM
    if (str_starts(nome_arquivo, "ncm_")) {
      dados <- dados |>
        mutate(no_cuci_sec = str_to_title(no_cuci_sec))  # Padroniza o título
    }
...
# 1. Função para carregar e processar dados de exportação
m_exportacao_reativo <- reactive({
  # Definir schema para garantir consistência
  schema <- schema(
    no_pais = string(),
    no_bloco = string(),
    sigla_uf = string(),
  ...
  # Carregar dados
  dados <- open_dataset("dados/m_exportacao_agrupado.parquet", schema = schema) |>
    collect() |>
    mutate(
      ...
      nome_mes = factor(nome_mes, levels = c("jan.", "fev.", "mar.", "abr.", "maio", "jun.",
                                             "jul.", "ago.", "set.", "out.", "nov.", "dez.")),
      no_pais = if_else(no_pais == "Países Baixos (Holanda)", "Holanda", no_pais),
...
# 2. Função para carregar e processar dados de importação
m_importacao_reativo <- reactive({
  ...
  dados <- open_dataset("dados/m_importacao_agrupado.parquet", schema = schema) |>
    collect() |>
    mutate(
...
criar_acumulado <- function(m_exportacao, m_importacao, variavelY) {
  ...
  exportacoes <- bind_rows(
    processar_dados(m_exportacao(), "Maranhão"),
    processar_dados(m_exportacao(), "Nordeste"),
    processar_dados(m_exportacao(), "Brasil")
  ) |>
    rename(exp_valor = valor)
  
  importacoes <- bind_rows(
    processar_dados(m_importacao(), "Maranhão"),
    processar_dados(m_importacao(), "Nordeste"),
    processar_dados(m_importacao(), "Brasil")
  ) |>
