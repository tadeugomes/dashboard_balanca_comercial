...
```{r}
#|panel: fill
highcharter::highchartOutput("balanca_anual") 
```
...
```{r}
#|panel: fill
DT::DTOutput("tabela_exp_produtos")
```
...
```{r}
#| panel: fill
#| label: "ui_destinos_anual"
highcharter::highchartOutput("destinos_anual") 
```
...
```{r}
#|panel: fill
DT::DTOutput("tabela_imp_produtos")
...
```{r}
#| panel: fill
#| label: "ui_origens_anual"
highcharter::highchartOutput("origens_anual") 
```
# O Porto
...
ncm_data_loader <- function(tipo, uf) {
  schema <- schema(
    no_pais = string(),
    no_uf = string(),
    no_regiao = string(),
...
criar_acumulado <- function(ncm_exportacao_reativo, ncm_importacao_reativo, variavelY) {
  ...
  # Correção do OOM: o group_by/summarise roda no Arrow (sem collect() antes),
  # só o resultado agregado é materializado. O ramo do `if` precisa ser
  # escolhido em R ANTES de montar a consulta Arrow, porque `if`/`else` dentro
  # de summarise() não é suportado pelo backend Arrow.
  processar_dados <- function(dados, uf) {
    agregado <- if (variavelY == "Valor (em milhões de dólares)") {
      dados |>
        group_by(ano, mes, nome_mes) |>
        summarise(valor = sum(valor_fob_dolar, na.rm = TRUE), .groups = "drop")
    } else {
      dados |>
        group_by(ano, mes, nome_mes) |>
        summarise(valor = sum(peso_liquido_kg, na.rm = TRUE), .groups = "drop")
    }
    agregado |>
      collect() |>
      mutate(uf = uf) |>
      # O Arrow não garante a ordem das linhas que devolve de um group_by, e
      # antes da correção do OOM a ordem vinha "de graça" porque o collect()
      # acontecia antes da agregação. Sem este arrange a tabela da aba Resumo
      # sai com os meses embaralhados e o gráfico mensal vira um emaranhado de
      # linhas cruzadas — foi o que aconteceu em produção em 06/08/2026.
      arrange(ano, mes)
  }
...
funcao_ncm_agrupado  <- function(dados, coluna_agrupamento, tipo) {
  ...
  # Correção do OOM: filtro, group_by e summarise rodam no Arrow; só o
  # resultado agregado é coletado. A coluna `uf` (constante por chamada) é
  # criada com mutate() depois do collect(), porque o Arrow não aceita
  # coluna constante criada dentro de group_by().
  processar_nivel <- function(dados, uf_valor, coluna_agrupamento) {
    dados |>
      filter(no_uf == uf_valor | no_regiao == uf_valor | uf_valor == "Brasil") |>
      group_by(ano, mes, nome_mes, {{coluna_agrupamento}}) |>
      summarise(
  ...
  # Filtrar dados para Maranhão, Nordeste e Brasil (fica no Arrow: só reduz
  # linhas, não materializa nada em memória)
...
output$tabelaBrasil_Maranhao <- DT::renderDT({
  ...
  dados_filtrados <-
    acumulado_reativo() |>
    dplyr::filter(ano == ANO_REF & uf == input$SelecaoUF) |>
  ...
  DT::datatable(
    ...
    caption = htmltools::tags$caption(
      ...
      htmltools::tags$p(paste0("Balança Comercial de ", ANO_REF, " do "), input$SelecaoUF, ", valores ", extrair_texto_entre_parenteses(input$variavelY))
    ) 
  ) |>
    DT::formatCurrency(
...
acumulado1 <- 
acumulado_reativo() |> 
  dplyr::filter(ano %in% c(max(ano), max(ano) - 1)) |>
  tidyr::pivot_longer(
...
acumulado1 <- 
 acumulado1 |>  dplyr::filter(uf == input$SelecaoUF) 
  ...
  # cat("Tabela Balança do ano de referência\n")
...
```{r}
#| context: server
#| label: fig-balanca_anual

output$balanca_anual <- highcharter::renderHighchart({
...
tooltip <- list(
  ...
  highcharter::hc_title(
    text = paste(paste0("Evolução da balança comercial anual (", ANO_MIN, "-", ANO_REF, ") do"),
...
```{r}
#| context: server
#| label: "exportacoes_prod_ref"

output$tabela_exp_produtos <- DT::renderDT({
  
 ############# DADOS ################ 
ncm_exportacao_agrupado <- dplyr::bind_rows(
    #Maranhão
    # Correção do OOM: filtro e agregação rodam no Arrow; o collect() vem
    # só depois, sobre o resultado já agregado. `uf` vira mutate() pós-collect
    # porque o Arrow não aceita coluna constante criada dentro de group_by().
    ncm_exportacao_reativo() |>
      dplyr::filter(
        no_uf == "Maranhão"
      ) |>
      dplyr::group_by(ano, mes, nome_mes, no_cuci_grupo) |>
      dplyr::summarise(
          valor_fob_dolar = sum(valor_fob_dolar, na.rm = TRUE),
          peso_liquido_kg = sum(peso_liquido_kg, na.rm = TRUE),
        .groups = "drop"
    ...
    ncm_exportacao_reativo() |>
      ...
      collect() |>
      dplyr::mutate(uf = "Nordeste"), # <--- uf = "Nordeste"
    ...
    ncm_exportacao_reativo() |>
      ...
      collect() |>
      dplyr::mutate(uf = "Brasil") # <--- uf = "Brasil"
  ) |>
  arrange(ano, mes)
...
exp_br_produtos_ref <- 
  ncm_exportacao_agrupado |> 
  dplyr::filter(ano == ANO_REF & no_cuci_grupo %in% lbls & 
                uf == input$SelecaoUF
                # uf == "Maranhão"
                ) |> 
...
# print(dplyr::glimpse(exp_br_produtos_ref))
#  
...
if ("Total" %in% exp_br_produtos_ref$nome_mes) {
  # Encontra a linha do Total
  total_row_index <- which(exp_br_produtos_ref$nome_mes == "Total")
  
  # Extrai os valores (excluindo a coluna nome_mes) e ordena
  total_values <- exp_br_produtos_ref[total_row_index, -1]
  colunas_ordenadas <- names(total_values)[order(as.numeric(total_values), decreasing = TRUE)]
  
   exp_br_produtos_ref <- exp_br_produtos_ref[, c("nome_mes", colunas_ordenadas)]
...
######### TABELA ########################################
  ...
  titulo <- htmltools::tags$caption(
    ...
    htmltools::tags$p(paste0("Os principais produtos exportados em ", ANO_REF))
  )
  legenda <- htmltools::tags$caption(
    style = 'caption-side: bottom; text-align: left;', # Legenda à esquerda embaixo
    htmltools::tags$p(paste0("Fonte: Secretaria de Comércio Exterior - SECEX (", ANO_REF, ")"))
  )
...
exp_br_produtos_ref |>
  DT::datatable(
    ...
    colnames = c("Mês", colnames(exp_br_produtos_ref)[-1]), # Define os nomes das colunas
    caption = cabecalho,
    escape = FALSE # Permite HTML na tabela (para o rodapé estilizado)
  ) |>
  DT::formatCurrency(
    columns = colnames(exp_br_produtos_ref)[-1], # Formata todas as colunas menos a primeira
    currency = "", mark = ".", dec.mark = ",", digits = 2
  ) |>
  DT::formatStyle(
    columns = 0:ncol(exp_br_produtos_ref), # Aplica o estilo a todas as colunas
    target = "row",
    backgroundColor = DT::styleEqual(seq(1, nrow(exp_br_produtos_ref), 2), "#f2f2f2") # Cor de fundo alternada
  ) |>
  DT::formatStyle(
    columns = 0:ncol(exp_br_produtos_ref), # Aplica o estilo a todas as colunas
    target = "row",
    fontWeight = DT::styleEqual(nrow(exp_br_produtos_ref), "bold") # Última linha em negrito
  )  
  })
...
```{r}
#| context: server
#| label: "server_destinos_anual"

output$destinos_anual <- highcharter::renderHighchart({
...
# print(exp_pais_periodo)
# cat("Nome das colunas\n")
# print(colnames(exp_pais_periodo))
...
highcharter::hchart(
  ...
  highcharter::hc_title(
    text = paste("Os Principais Destinos das Exportações do ", 
                 input$SelecaoUF, paste0(" entre ", ANO_MIN, "-", ANO_REF, ", valores "),
                 extrair_texto_entre_parenteses(input$variavelY)),
    style = list(fontSize = "20px", fontWeight = "bold"))  
...
```{r}
#| context: server
#| label: "importacoes_prod_ref"

output$tabela_imp_produtos <- DT::renderDT({
...
ncm_importacao_agrupado <- 
 ncm_importacao_agrupado <- dplyr::bind_rows(
        #Maranhão
        # Correção do OOM: filtro e agregação rodam no Arrow; o collect() vem
        # só depois, sobre o resultado já agregado. `uf` vira mutate()
        # pós-collect porque o Arrow não aceita coluna constante criada
        # dentro de group_by().
...
# print(dplyr::glimpse(imp_br_produtos_ref))
#  
# adicionando linha de total
imp_br_produtos_ref <-
imp_br_produtos_ref |> 
...
# Reordenar colunas baseado na linha 'Total'
if ("Total" %in% imp_br_produtos_ref$nome_mes) {
  # Encontra a linha do Total
  total_row_index <- which(imp_br_produtos_ref$nome_mes == "Total")
  
  # Extrai os valores (excluindo a coluna nome_mes) e ordena
  total_values <- imp_br_produtos_ref[total_row_index, -1]
  colunas_ordenadas <- names(total_values)[order(as.numeric(total_values), decreasing = TRUE)]
  
   imp_br_produtos_ref <- imp_br_produtos_ref[, c("nome_mes", colunas_ordenadas)]
...
######### TABELA ########################################
  ...
  titulo <- htmltools::tags$caption(
    style = 'caption-side: top; text-align: center; font-size: 20px; font-weight: bold;', # Título centralizado em cima
    htmltools::tags$p(paste0("Os principais produtos importados em ", ANO_REF))
  )
  legenda <- htmltools::tags$caption(
    style = 'caption-side: bottom; text-align: left;', # Legenda à esquerda embaixo
    htmltools::tags$p(paste0("Fonte: Secretaria de Comércio Exterior - SECEX (", ANO_REF, ")"))
...
imp_br_produtos_ref |>
  DT::datatable(
    ...
    rownames = FALSE, # Remove os números das linhas
    colnames = c("Mês", colnames(imp_br_produtos_ref)[-1]), # Define os nomes das colunas
    caption = cabecalho,
    escape = FALSE # Permite HTML na tabela (para o rodapé estilizado)
  ) |>
  DT::formatCurrency(
    columns = colnames(imp_br_produtos_ref)[-1], # Formata todas as colunas menos a primeira
    currency = "", mark = ".", dec.mark = ",", digits = 2
  ) |>
  DT::formatStyle(
    columns = 0:ncol(imp_br_produtos_ref), # Aplica o estilo a todas as colunas
    target = "row",
    backgroundColor = DT::styleEqual(seq(1, nrow(imp_br_produtos_ref), 2), "#f2f2f2") # Cor de fundo alternada
  ) |>
  DT::formatStyle(
    columns = 0:ncol(imp_br_produtos_ref), # Aplica o estilo a todas as colunas
    target = "row",
    fontWeight = DT::styleEqual(nrow(imp_br_produtos_ref), "bold") # Última linha em negrito
  )  
  })
...
```{r}
#| context: server
#| label: "server_origens_anual"

output$origens_anual <- highcharter::renderHighchart({
...
# cat("Print depois do slice_max\n")
# print(imp_pais_periodo)
# cat("Nome das colunas\n")
# print(colnames(imp_pais_periodo))
