...
ler_dados <- function(nome_arquivo) {
    nomes_arquivos <- c("ncm_exportacao", "ncm_importacao", "m_exportacao", "m_importacao")
    if (!(nome_arquivo %in% nomes_arquivos)) {
        stop(paste0("Nome de arquivo inválido. Escolha entre: ", paste(nomes_arquivos, collapse = ", ")))
    }
    caminho_arquivo <- paste0("dados/", nome_arquivo, ".parquet")
    
    tryCatch({
        dados <- arrow::read_parquet(caminho_arquivo, col_select = everything())
        
        # if (nome_arquivo %in% c("ncm_exportacao", "ncm_importacao")) {
        #     dados <- dados %>%
        #         dplyr::mutate(no_cuci_sec = stringr::str_to_title(no_cuci_sec))
        # }
        return(dados)
    }, error = function(e) {
        message(paste("Erro ao ler o arquivo:", e))
...
knitr::opts_chunk$set(message = FALSE, warning = FALSE, echo = FALSE, dpi = 400)




# se a pasta dados não estiver criada, então criar a pasta dados
if (!dir.exists("dados")) {dir.create("dados", showWarnings = FALSE)}
...
format_number_with_scale <- function(x, scale_breaks = c(1e3, 1e6, 1e9, 1e12), scale_suffixes = c(" Mil", " Mi", " Bi", " Tri"), big.mark = ".", decimal.mark = ",") {
    
    format_single <- function(x_val, i_val) {
        if (is.na(x_val) || is.infinite(x_val)) {
            return(NA)
        }
        if (i_val > 0) {
            suffix <- scale_suffixes[i_val]
            scaled_x <- x_val / scale_breaks[i_val]
            paste0(format(round(scaled_x, 2), big.mark = big.mark, decimal.mark = decimal.mark, scientific = FALSE, nsmall = 2), suffix) # Modificado aqui
        } else {
            format(round(x_val, 2), big.mark = big.mark, decimal.mark = decimal.mark, scientific = FALSE, nsmall = 2) # Modificado aqui
        }
    }
    
    i <- findInterval(abs(x), scale_breaks)
    mapply(format_single, x, i)
}
...
######################### M_EXPORTACAO #########################################
m_exportacao_reativo <- reactive({
  # m_exportacao <- ler_dados("m_exportacao") 
  schema  <- 
  arrow::schema(
    no_pais = arrow::string(),
    no_bloco = arrow::string(),
    sigla_uf = arrow::string(),
    no_uf = arrow::string(),
    no_regiao = arrow::string(),
    no_mun_min = arrow::string(),
    # interger
    ano = arrow::int64(),
    mes = arrow::int64(),
    # double
    peso_liquido_kg = arrow::float64(),
    valor_fob_dolar = arrow::float64() 
  )


m_exportacao <- 
  arrow::open_dataset(
    sources = "dados/m_exportacao_agrupado.parquet",
    schema = schema
  ) |> dplyr::collect()
...
m_exportacao <-
    ...
    dplyr::mutate(
      ...
      no_pais = dplyr::if_else(no_pais == "Países Baixos (Holanda)", "Holanda", no_pais)
    ) |> 
  mutate(
    valor_fob_dolar = valor_fob_dolar/10^6,
    peso_liquido_kg = peso_liquido_kg /10^9
  )
...
m_exportacao <-
    m_exportacao |>
    dplyr::mutate(
      dplyr::across(tidyselect::where(is.character), ~ dplyr::case_when(is.na(.x) ~ "", TRUE ~ .x)),
...
m_exportacao <-
    ...
    dplyr::mutate(
      no_regiao =
        dplyr::case_when(
          no_regiao == "REGIAO NORDESTE" ~ "Nordeste",
          no_regiao == "REGIAO SUDESTE" ~ "Sudeste",
          no_regiao == "REGIAO SUL" ~ "Sul",
          no_regiao == "REGIAO NORTE" ~ "Norte",
          no_regiao == "REGIAO CENTRO OESTE" ~ "Centro-Oeste",
          no_regiao == "REGIAO NAO DECLARADA" ~ "Não Declarada"
        )
    )
...
m_importacao <- 
  arrow::open_dataset(
    sources = "dados/m_importacao_agrupado.parquet",
    schema = schema
  )  |> dplyr::collect()
...
################### ACUMULADO: M_EXPORTACAO E M_IMPORTACAO ######################
criar_acumulado <- function(m_exportacao, m_importacao, variavelY) {


  # Calcula as exportações
  exportacoes <- dplyr::bind_rows(
    #Maranhão
    m_exportacao() |>
      dplyr::filter(
        no_uf == "Maranhão"
      ) |>
      dplyr::group_by(ano, mes, nome_mes, uf = no_uf) |>
      dplyr::summarise(
        exp_valor = ifelse(
          variavelY == "Valor (em milhões de dólares)",
    ...
    m_exportacao() |>
      ...
      dplyr::group_by(ano, mes, nome_mes, uf = "Nordeste") |> # <--- uf = "Nordeste"
      dplyr::summarise(
        exp_valor = ifelse(
          variavelY == "Valor (em milhões de dólares)",
          sum(valor_fob_dolar, na.rm = TRUE),
    ...
    # Brasil
    m_exportacao() |>
      dplyr::group_by(ano, mes, nome_mes, uf = "Brasil") |> # <--- uf = "Brasil"
      dplyr::summarise(
        exp_valor = ifelse(
          variavelY == "Valor (em milhões de dólares)",
          sum(valor_fob_dolar, na.rm = TRUE),
  ...
  # Calcula as importações
  importacoes <- dplyr::bind_rows(
    # Maranhão
    m_importacao() |>
      dplyr::filter(
          no_uf == "Maranhão"
    ...
    m_importacao() |>
      ...
      dplyr::group_by(ano, mes, nome_mes , uf = "Nordeste") |>
      dplyr::summarise(
        imp_valor = ifelse(
          variavelY == "Valor (em milhões de dólares)",
          sum(valor_fob_dolar, na.rm = TRUE),
    ...
    m_importacao() |>
      ...
      dplyr::group_by(ano, mes, nome_mes, uf = "Brasil") |>
      dplyr::summarise(
        imp_valor = ifelse(
          variavelY == "Valor (em milhões de dólares)",
          sum(valor_fob_dolar, na.rm = TRUE),
  ...
  # Junta as exportações e importações
  acumulado <- exportacoes |>
    dplyr::left_join(importacoes, by = c("ano", "mes", "nome_mes", "uf")) |>
    dplyr::mutate(
...
# Objeto reativo para 'acumulado'
acumulado_reativo <- reactive({
  criar_acumulado(m_exportacao_reativo, m_importacao_reativo, input$variavelY)
})


#################################### NCM_EXPORTACAO #####################################
ncm_exportacao_reativo <- shiny::reactive({
schema  <- 
  arrow::schema(
    no_pais = arrow::string(),
    no_bloco = arrow::string(),
    sigla_uf_ncm = arrow::string(),
    no_uf = arrow::string(),
    no_regiao = arrow::string(),
    no_via = arrow::string(),
    no_urf = arrow::string(),
    no_cuci_grupo = arrow::string(),
    no_cgce_n1 = arrow::string(),
    # interger
    ano = arrow::int64(),
    mes = arrow::int64(),
    # double
...
ncm_exportacao <- 
  arrow::open_dataset(
    sources = "dados/ncm_exportacao_agrupado.parquet",
    schema = schema
  ) |> dplyr::collect()
...
ncm_exportacao <- 
  ncm_exportacao |> 
  dplyr::mutate(
    dplyr::across(tidyselect::where(is.character), ~ dplyr::case_when(is.na(.x) ~ "", TRUE ~ .x)), # Para character
    dplyr::across(tidyselect::where(is.numeric), ~ tidyr::replace_na(.x, 0)) # Para numeric
  )
...
ncm_exportacao <-
  ...
  return(ncm_exportacao)
})
#################################### NCM_IMPORTACAO #####################################
ncm_importacao_reativo <- reactive({
schema  <- 
  arrow::schema(
    no_pais = arrow::string(),
    no_bloco = arrow::string(),
...
ncm_importacao <- 
  arrow::open_dataset(
    sources = "dados/ncm_importacao_agrupado.parquet",
    schema = schema
  ) |> dplyr::collect()
...
ncm_importacao <-
  ...
  return(ncm_importacao)
})



extrair_texto_entre_parenteses <- function(texto) {
  # Encontra todas as ocorrências de texto dentro de parênteses
  resultados <- stringr::str_extract_all(texto, "(?<=\\().*?(?=\\))")[[1]]
...
funcao_ncm_exportacao_agrupado <- function(dados, coluna_agrupamento) {
  ...
  processar_nivel <- function(dados, uf_valor, coluna_agrupamento) {
    dados |>
      dplyr::group_by(ano, mes, nome_mes, uf = uf_valor, {{coluna_agrupamento}}) |>  # Use {{coluna_agrupamento}} aqui
      dplyr::summarise(
 ...
 # Filtrando o data frame dados antes de agrupar (corrigido aqui)
  dados_filtrado <- dados |> dplyr::filter(no_uf %in% c("Maranhão", "Brasil") | no_regiao == "Nordeste")
  


  resultado <- dplyr::bind_rows(
    processar_nivel(
      dados_filtrado |> dplyr::filter(no_uf == "Maranhão"),
      "Maranhão", coluna_agrupamento = no_pais   # Corrigido aqui
    ),
    processar_nivel(
      dados_filtrado |> dplyr::filter(no_regiao == "Nordeste"),
      "Nordeste", coluna_agrupamento = no_pais   # Corrigido aqui
    ),
    processar_nivel(
      dados_filtrado, # Brasil não precisa de filtro
      "Brasil", coluna_agrupamento = no_pais   # Corrigido aqui
    )
  )
  return(resultado)
}
