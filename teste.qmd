...
ncm_importacao <- 
ncm_importacao |> 
  dplyr::group_by(no_pais, no_uf, no_regiao,
           no_cuci_grupo, ano, mes,
           nome_mes) |> 
  dplyr::summarise(
...
# 3. Função para converter números em meses
nome_mes <- function(x) {
  meses <- c(
  ...
  ifelse(x >= 1 & x <= 12, meses[x], "mês não encontrado")
}
...
# Função para carregar dados de forma eficiente com memoise
carregar_dados <- memoise::memoise(function(nome_arquivo) {
  ...
  # Caminho do arquivo
  caminho_arquivo <- file.path("dados", paste0(nome_arquivo, ".parquet"))

  # Leitura segura com tratamento de erros
  tryCatch({
    dados <- arrow::read_parquet(
      caminho_arquivo,
      col_select = dplyr::everything() # Seleciona todas as colunas
    )

    # Transformações específicas para arquivos NCM
    if (stringr::str_starts(nome_arquivo, "ncm_")) {
      dados <- dados |>
        dplyr::mutate(no_cuci_sec = stringr::str_to_title(no_cuci_sec)) # Padroniza o título
    }

    return(dados)
  }, error = function(e) {
    message("Erro ao ler o arquivo ", nome_arquivo, ": ", e$message)
    return(NULL) # Retorna NULL em caso de erro
  })
})

# Função para processar dados de comércio exterior
processar_dados_comercio <- function(dados, uf, tipo, variavelY) {
  dados |>
    dplyr::filter(no_uf == uf | no_regiao == uf | uf == "Brasil") |>
    dplyr::group_by(ano, mes, nome_mes, uf = uf) |>
    dplyr::summarise(
...
calcular_acumulado <- function(dados_exportacao, dados_importacao, variavelY) {
  exportacoes <- dplyr::bind_rows(
    processar_dados_comercio(dados_exportacao, "Maranhão", "exportacao", variavelY),
    processar_dados_comercio(dados_exportacao, "Nordeste", "exportacao", variavelY),
    processar_dados_comercio(dados_exportacao, "Brasil", "exportacao", variavelY)
  ) |>
    dplyr::rename(exp_valor = valor)

  importacoes <- dplyr::bind_rows(
    processar_dados_comercio(dados_importacao, "Maranhão", "importacao", variavelY),
    processar_dados_comercio(dados_importacao, "Nordeste", "importacao", variavelY),
    processar_dados_comercio(dados_importacao, "Brasil", "importacao", variavelY)
  ) |>
    dplyr::rename(imp_valor = valor)
...
# Função para carregar dados NCM com memoise
carregar_dados_ncm <- memoise::memoise(function(tipo) {
  schema <- arrow::schema(
  ...
  open_dataset(paste0("dados/ncm_", tipo, "_agrupado.parquet"), schema = schema) |>
    collect() |>
    dplyr::mutate(
      nome_mes = nome_mes(mes),
      nome_mes = factor(nome_mes, levels = c("jan.", "fev.", "mar.", "abr.", "maio", "jun.",
                                             "jul.", "ago.", "set.", "out.", "nov.", "dez.")),
      no_pais = dplyr::if_else(no_pais == "Países Baixos (Holanda)", "Holanda", no_pais),
      valor_fob_dolar = valor_fob_dolar / 1e6,
      peso_liquido_kg = peso_liquido_kg / 1e9
    ) |>
    dplyr::mutate(
      across(where(is.character), ~coalesce(.x, "")),
      across(where(is.numeric), ~coalesce(.x, 0))
    ) |>
    dplyr::mutate(
      no_regiao = dplyr::case_when(
...
# Objetos reativos para dados de exportação e importação
dados_exportacao_reativo <- reactive({
  carregar_dados("m_exportacao_agrupado") |>
    dplyr::mutate(
      nome_mes = nome_mes(mes), # Função auxiliar para converter números em meses
      nome_mes = factor(nome_mes, levels = c("jan.", "fev.", "mar.", "abr.", "maio", "jun.",
                                             "jul.", "ago.", "set.", "out.", "nov.", "dez.")),
      no_pais = dplyr::if_else(no_pais == "Países Baixos (Holanda)", "Holanda", no_pais),
      valor_fob_dolar = valor_fob_dolar / 1e6, # Converter para milhões
      peso_liquido_kg = peso_liquido_kg / 1e9 # Converter para bilhões
    ) |>
    dplyr::mutate(
      across(where(is.character), ~coalesce(.x, "")), # Substituir NA em colunas de texto
      across(where(is.numeric), ~coalesce(.x, 0)) # Substituir NA em colunas numéricas
    ) |>
    dplyr::mutate(
      no_regiao = dplyr::case_when(
        no_regiao == "REGIAO NORDESTE" ~ "Nordeste",
...
dados_importacao_reativo <- reactive({
  carregar_dados("m_importacao_agrupado") |>
    dplyr::mutate(
      nome_mes = nome_mes(mes),
      nome_mes = factor(nome_mes, levels = c("jan.", "fev.", "mar.", "abr.", "maio", "jun.",
                                             "jul.", "ago.", "set.", "out.", "nov.", "dez.")),
...
# Objeto reativo para dados acumulados
acumulado_reativo <- reactive({
  calcular_acumulado(dados_exportacao_reativo, dados_importacao_reativo, input$variavelY)
})

# Objetos reativos para dados NCM
dados_ncm_exportacao_reativo <- reactive({
  carregar_dados_ncm("exportacao_agrupado")
})

dados_ncm_importacao_reativo <- reactive({
  carregar_dados_ncm("importacao__agrupado")
})

# Função auxiliar para extrair texto entre parênteses
extrair_texto_entre_parenteses <- function(texto) {
  stringr::str_extract_all(texto, "(?<=\\().*?(?=\\))")
}
